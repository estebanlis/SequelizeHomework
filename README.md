...
Vamos a generar tres modelos `Character`, `Ability` y `Role`. Cada uno de ellos tiene un archivo dentro de la carpeta models que tendrán que ir completando segun la siguiente información:
...
Cada uno de ellos verán que está importado en el archivo `index.js` de la carpeta `db`:
...
Y luego los incorporamos a la instancia de sequelize que habíamos creado:
...
Por último en el export del final al tener la linea `...db.models` nos va a permitir requerir dichos modelos en el resto de los archivos que los necesitemos de la siguiente forma (por ejemplo para `Characters`):

```js
const { Character } = require('./db'); // Ajustar la ruta según corresponda
```

### Rutas

Van a tener ya los middlewares para cada modelo configurados, la idea es vayan agregando las rutas en esos archivos dentro de la carpeta `middlewares`.
...
Debe retornar todos los personajes que se encuentren creados en la base de datos. Además este endpoint debe aceptar por query un valor de una raza y una edad para filtrar los resultados, por ejemplo: GET /character?race=human&age=27 y debe filtrar usando un OR (Es decir traerla todos aquellos que matcheen con la raza o con la edad).

<img style="float: left; padding-right: 10px" src="./img/extra.png">

Adicionalmente pueden hacer que reciba por query los atributos que quiera devolver en el caso de no querer todos, por ejemplo: GET /character?name=true&race=true&hp=true

<img style="float: left; padding-right: 10px" src="./img/extra.png">

Adicionalmente pueden hacer que de forma dinámica pueda filtrar por cualquiera de los atributos del modelo de Character y no solamente por race y hp, tengan en cuenta que pueden pasarle la cantidad de parámetros que quieran, desde ninguno hasta todos.
